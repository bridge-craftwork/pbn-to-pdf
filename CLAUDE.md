...
├── parser/              # Reading a PBN file into the render model
│   ├── pbn.rs           # bridge-encodings' read_pbn, adapted to model::Board
│   ├── commentary.rs    # Formatted text with suit codes (not modeled upstream)
│   └── header.rs        # Bridge Composer % directives (not modeled upstream)
...
**Parsing lives in `bridge-encodings`.** `parse_pbn` is a thin adaptation of
`bridge_encodings::pbn::read_pbn` onto the render model; this crate no longer
carries a PBN parser of its own. What stays here is what a general reader has no
business modelling: rich commentary spans, `%` header directives, Bridge
Composer display flags, and the typed contract.

Three producer quirks are handled upstream rather than here, and each one fails
...
- **`AP`** stands for the three passes that end an auction.

A fourth quirk is not a quirk at all, and the standard is worth quoting for it:
`+` **replaces the next call to be made** (3.5), so it holds a call's place
rather than merely closing the section, and a NAG after it annotates that
placeholder. Bridge Composer writes `1D X Pass + $2` and renders the South cell
as "??" — the placeholder and its "?" — which is how the reader models it and
what `Call::Continue` draws here. The same goes for `-` and `+` in a Play
section: each holds its seat, because the opening leader's card is the first on
the line. Skipping them slides every later card one seat left, which turns a
line whose lead is *not* on record (`- - - HJ`) into a confident, wrong claim
about the opening lead.



Key PBN elements the reader handles:
